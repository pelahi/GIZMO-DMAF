...
// DM ANNIHILATION at GAS
	...
	MyDouble k_rho_dm2_over_rho;
	MyDouble cross_section = All.CrossSection;		// velocity averaged cross section in cm^3/s
	MyDouble dm_particle_mass = (MyFloat) All.ParticleMass; // DM Particle mass												
#endif

// INJECT ENERGY
#ifdef INJECT_ENERGY
    MyFloat energy_source = All.EnergySource;
	MyFloat energy_ID = All.EnergyID;
#endif

// DEBUG DEBUG DEBUG for FORCE_EQUAL_TIMESTEP
/*	double energy_exact = 1.4751888e-07;
	double energy_inject = 0.0;
	for(i = 0; i < NumPart; i++)
	{
		if(!P[i].Type)
		{
			...
			{
				energy_inject += SphP[i].DMAF_Dtu[k] * P[i].Mass;
			}
		}		
	}
	printf("Energy meant to be injected: %g, energy injected: %g\n", energy_exact, energy_inject); */
...
// DM ANNIHILATION after hydro loop
#ifdef DMANNIHILATION
#ifdef DEBUG_DMANNIHILATION
			SphP[i].DtInternalEnergy += (P[i].DensityDM / P[i].Mass);			
#else
			k_rho_dm2_over_rho = P[i].DensityDM * P[i].DensityDM / SphP[i].Density * All.cf_a3inv * All.HubbleParam; // should be in  10^10 Msol/kpc^3
			// NOTE: there should be two h's as well but they appear in the next line as part of the conversion to sec/h, so they cancel out. 
			heat_source = All.UnitTime_in_s * k_rho_dm2_over_rho * C_km_s * C_km_s * CONVERT_FAC * cross_section / dm_particle_mass; // should be in (km/s)^2 / internal time
			// Add internal energy created by DM annihilation
			SphP[i].DtInternalEnergy += (1.0 / All.cf_atime * heat_source); 
			SphP[i].DMAF_Dtu += (1.0 / All.cf_atime * heat_source);
#endif			
#endif 

// INJECT ENERGY after hydro loop
#ifdef INJECT_ENERGY
			if(P[i].ID == energy_ID)
			{
				SphP[i].DtInternalEnergy += (energy_source / P[i].Mass);	
				SphP[i].DMAF_Dtu += (1.0 / All.cf_atime * heat_source);
			}
#endif
...
// DM ANNIHILATION at gas: zero out SphP[i].DMAF_Dtu
// for DM ANNIHILATION at DM, this is done per DM time bin in timestep.c when max_active_time_bin is calculated
...
/*
#ifdef INJECT_ENERGY
int energy_num_part = 0;
printf("energy rad: %g\n", All.EnergyRad);
while(energy_num_part != All.EnergyNgb)
{
	energy_num_part = 0;
	for(i = FirstActiveParticle; i >= 0; i = NextActiveParticle[i])
	{
	    if(P[i].Type==0)
		{
			if((P[i].Pos[0]-All.EnergyLoc)*(P[i].Pos[0]-All.EnergyLoc) < All.EnergyRad*All.EnergyRad)
			{
				energy_num_part++;
			}
		}
	}
	if(energy_num_part < All.EnergyNgb)
	{
		All.EnergyRad *= 1.4;
	} else if(energy_num_part > All.EnergyNgb)
	{
		All.EnergyRad *= 0.9;
	}
	//printf("Energy num part.: %i\n, Energy rad.: %g\n", energy_num_part, All.EnergyRad);
}
#endif
*/
