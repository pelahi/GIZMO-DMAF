  ...
  for(i = 0; i < NumPart; i++)
    {
      ...
      if(All.ComovingIntegrationOn)
	{
	  dt_entr = dt_hydrokick = (All.Ti_Current - (P[i].Ti_begstep + dt_step / 2)) * All.Timebase_interval / All.cf_hubble_a;
      ...
      for(j = 0; j < 3; j++)
	{
	  ...
	  if(P[i].Type == 0)
	    vel[j] += SphP[i].HydroAccel[j] * dt_hydrokick * All.cf_atime; /* convert from physical to code vel */
	}
      if(P[i].Type == 0)
	{
