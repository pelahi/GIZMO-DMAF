...
// For INJECT_ENERGY_DM: set DMANNIHILATION_DM
#ifdef INJECT_ENERGY_DM
#define DMANNIHILATION_DM
#endif

// Disable rescaling of source terms to match current cosmological factors at gas
#ifndef DMAF_DISABLE_BIAS_CORRECTION
#define DMAF_CORRECT_BIAS
#endif
...
#define  C_km_s      299792.458 /* speed of light in km/s */
#define  CONVERT_FAC 379.66416345 /* conversion factor from GeV c^-2 cm^-3 to 10^10 M_sol kpc^-3 (computed with astropy) */
#endif
...
#define MAXLEN_OUTPUTLIST 1200	/*!< maximum number of entries in output list */
...
#define CPU_DMDENSCOMPUTE  40
#define CPU_DMDENSWAIT     41
#define CPU_DMDENSCOMM     42
#define CPU_DMDENSMISC     43
#define CPU_GASDENSCOMPUTE 44
#define CPU_GASDENSWAIT    45
#define CPU_GASDENSCOMM    46
#define CPU_GASDENSMISC    47
#define CPU_VELSHEARCOMP   48
...
// DM ANNIHILATION
#if defined(DMANNIHILATION) || defined(DMANNIHILATION_DM)
	double CrossSection;	   /*!< DM velocity cross section */
	double ParticleMass; 	   /*!< DM particle mass */
#endif

// INJECT ENERGY
#if defined(INJECT_ENERGY) || defined(INJECT_ENERGY_DM)
	double EnergySource;  /*< Magnitude of energy being injected */
	// double EnergyLoc; /*< x location where energy is being injected (y = z = 0) */
	// int EnergyNgb;    /*< desired number of neighbours for energy injection */
	// double EnergyRad; /*< radius for distribution of energy */
	int EnergyID; /*< ID of the particle that gets the energy injected */
#endif
...
// DM ANNIHILATION
#if defined(DMANNIHILATION) || defined(DMANNIHILATION_DM)
	MyDouble DensityDM;		           /*!< current mass density of particle */
    MyFloat HsmlDM;			           /*!< current smoothing length */
    MyFloat NumNgbDM;                  /*!< weighted number of neighbours found */
	MyFloat DhsmlNgbFactorDM;		   /*!< neighbour factor */
	MyFloat Particle_DivVelDM; 		   /*!< divergence of velocity */	
#endif
#ifdef DMANNIHILATION_DM
	MyDouble DensAroundDM; 			   /*!< gas density around DM particle */	
#ifndef DMAF_DENSITY_WEIGHTED
	MyDouble AreaSumDM; 			   /*!< sum of all solid angles of Ngb particles, used for normalising the energy injection */
#endif
	int DMAF_MaxTimebin; 			   /*!< GAS: maximum time step for gas particle in order to not exceed the time step of injecting DM particles */
									   /*!< DM: if DMAF_LIMIT_DM_TIMESTEP, this is also used for DM particles in order to set the maximum allowed time bin */
#ifndef DMAF_KEEP_DM_MASS
	MyDouble DMAF_dM; 				   /*!< Mass loss of DM particle */
#endif
#endif

// VWEB
#ifdef VWEB
	MyDouble VelShear[3][3];           /*!< velocity shear tensor */
#endif
...
#ifdef DMANNIHILATION_DM
	MyDouble DMAF_Dtu[TIMEBINS]; 				    /*!< energy injection rate due to DMAF, from each timebin of DM particles */	
	MyDouble DMAF_Dtu_tot; 							/*!< store the total energy injection rate separately */
#elif defined(DMANNIHILATION)
	MyDouble DMAF_Dtu;                              /*!< total energy injection rate due to annihilation */
#endif
